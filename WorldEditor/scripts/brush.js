...
export function brush(player){
    if(player.hasTag("worldeditor") && ev.itemStack.typeId.includes("blaze_powder") && toggle == 1 && brushMode.has(player.id) && player.isSneaking){
		brushMode.delete(player.id);
		player.sendMessage(`§aBrush mode disabled`)
	} else if(player.hasTag("worldeditor") && ev.itemStack.typeId.includes("blaze_powder") && toggle == 1 && brushMode.has(player.id)){
		try{
